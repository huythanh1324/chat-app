...
const SideDrawer = () => {
  ...
  const handleSearch = async() =>{
    ...
    try{
      ...
      const {data} = await axios.get(`https://127.0.0.1:5000/api/user?search=${search}`, config)
      setLoading(false)
  ...
  const accessChat = async(userId) =>{
    ...
    try{
      ...
      const {data} = await axios.post('https://chap-app-otxn.onrender.com/api/chat', {userId}, config)
